...
export function createConfigFile() {
    ...
    if (!fs.existsSync(venvPath)) {
        ...
        cp.exec(`python3 -m venv "${venvPath}"`, (error, stdout, stderr) => {
            ...
            console.log('Activate Command:', activateCommand);  // Log the activate command path

            const installCommand = process.platform === 'win32'
                ? `"${activateCommand}" && pip install browserstack-sdk`
                : `source "${activateCommand}" && pip install browserstack-sdk`;

            // Log the install command to verify
...
consoleLogs: info`;
        ...
        console.log('Activate Command:', activateCommand);  // Log activate path again

        const installCommand = process.platform === 'win32'
            ? `"${activateCommand}" && pip install browserstack-sdk`
            : `source "${activateCommand}" && pip install browserstack-sdk`;

        console.log('Install Command:', installCommand);  // Log the install command
